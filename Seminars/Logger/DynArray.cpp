...
namespace SDI
{
	...
	*/
	template<class ComponentType>
	void DynArray<ComponentType>::reserve(const unsigned int size)
	{
		assert(size >= size_);
	...
	*/
	template<class ComponentType>
	void DynArray<ComponentType>::remove(const unsigned int index)
	{
		assert(index < size_);
	...
	*/
	template<class ComponentType>
	void DynArray<ComponentType>::set(const unsigned int index, const ComponentType compType)
	{
	...
	*/
	template<class ComponentType>
	ComponentType& DynArray<ComponentType>::operator[](const unsigned int index)
	{
		assert(index >= 0 && index <= size_, "Out of bounds");
		if (index == size_)
		{
	...
	*/
	template<class ComponentType>
	ComponentType DynArray<ComponentType>::get(const unsigned int index) const
	{
		assert(size_ > index);
		return type_[index];
	}
